...
df = df[df['category_new'] == 3].squeeze(axis=1)

df.loc[:, 'Text_new'] = df['Text_new'].apply(clean_text).apply(remove_emoji)
...
plt.title("Top 20 most common words in user's review - Skill Usability")
sns.barplot(x=y,y=x,)
...
plt.title("Top 20 most common bigrams in user's review")
...
# This test is to find the key users reviews with "top bigrams words"
...
df.to_excel('usability_key.xlsx', index=False)           
...
wordcloud.to_file("wordcloud_C3.png")